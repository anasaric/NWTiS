...
public class KazneResurs {
	...
	public Response posttJsonDodajKaznu(@HeaderParam("Accept") String tipOdgovora, Kazna novaKazna) {
		...
		var kazne = pretvoriKaznaPost(novaKazna);
	...
	private Kazne pretvoriKaznaPost(Kazna kazna) {
		var kazne = new Kazne();
		...
		if (vozilo == null) {
			throw new IllegalArgumentException("Vozilo sa ID: " + kazna.getId() + " nije pronađeno.");
		}
	...
	private boolean provjeriPosluzitelja() {
		this.adresaKazne = Main.getKazneAdresa();
		this.mreznaVrataKazne = Main.getKazneMreznaVrata();
